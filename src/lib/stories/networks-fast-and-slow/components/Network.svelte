<script lang="ts">
	...
	// Initial layout and state (seeded once from props)
	let nodes_xy: Node[] = $state(untrack(() => radialLayout({ nodes, links, width, height: innerHeight as number })));
	...
	let selectedNeighbor: Node | null = $state(null);
